...
import org.apache.spark.streaming.kafka.filter.{Filter, ReceiverFilter}

import scala.collection.JavaConversions._
import scala.reflect.ClassTag

object SlowRateFilterKafkaUtils {
    /**
    ...
    def createStream(
                        ...
                        filterClass: Class[_ <: Filter] = classOf[ReceiverFilter]
                    ): ReceiverInputDStream[(String, String)] = {
        val kafkaParams = Map[String, String](
            "zookeeper.connect" -> zkQuorum, "group.id" -> groupId,
            "zookeeper.connection.timeout.ms" -> "10000")
        createStream[String, String, StringDecoder, StringDecoder](
            ssc, kafkaParams, topics, storageLevel,filterClass)
    }
    ...
    def createStream[K: ClassTag, V: ClassTag, U <: Decoder[_]: ClassTag, T <: Decoder[_]: ClassTag](
        ...
        //采用自定义的InputDStream
        new SlowRateFilterKafkaInputDStream[K, V, U, T](ssc, kafkaParams, topics, walEnabled, storageLevel,filterClass)
    }
    ...
    def createStream[K, V, U <: Decoder[_], T <: Decoder[_]](
        ...
        createStream[K, V, U, T](
            jssc.ssc, kafkaParams.toMap, Map(topics.mapValues(_.intValue()).toSeq: _*), storageLevel,filterClass)
    }
}

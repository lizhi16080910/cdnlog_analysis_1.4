...
import com.fastweb.cdnlog_analysis.DirCsDomainConf

/**
 * Created by lfq on 2016/7/25.
 */

class DirCsKafkaReceiver[K: ClassTag, V: ClassTag, U <: Decoder[_]: ClassTag, T <: Decoder[_]: ClassTag](
    ...
    def run() {
      ...
      try {
        ...
        while (streamIterator.hasNext()) {
          ...
          val message = msgAndMetadata.message().toString()

          val startIndex = message.indexOf("://") + 3
          if (startIndex > 3) {
            val endIndex = message.indexOf("/", startIndex)
            if (endIndex > 0) {
              val domain = message.substring(startIndex, endIndex)
              if (DirCsDomainConf.update().contains(domain)) {

...
/**
  ...
  */
private[streaming]
class SlowRateFilterReliableKafkaReceiver[
K: ClassTag,
V: ClassTag,
...
T <: Decoder[_]: ClassTag](
    ...
    override def onStart(): Unit = {
        ...
        topicMessageStreams.values.foreach { streams =>
            streams.foreach { stream =>
                messageHandlerThreadPool.submit(new MessageHandler(stream,filterClass))
            }
        }
    }
    ...
    private final class MessageHandler(stream: KafkaStream[K, V],filterClass: Class[_ <: Filter]) extends Runnable {
            ...
            while (!isStopped) {
                try {
                    ...
                    while (streamIterator.hasNext) {
                        ...
                        if(filter.filter(msgAndMetadata.message().asInstanceOf[String])){
                            storeMessageAndMetadata(msgAndMetadata)
                        }
                    }
                } catch {
                    case e: Exception =>

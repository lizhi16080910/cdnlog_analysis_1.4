...
object CdnlogPlatformAnalysis {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf()
      .setAppName("CdnlogPlatformFlowSizeCount")
   ...
   // "socket.timeout.ms" -> "60000"
    ...
    windowLine.map(x => {
      ...
      result.saveToEs("cdnlog.flowsize.platform.count.five." + indexPart.substring(0,8)+"/"+ indexPart)
    })

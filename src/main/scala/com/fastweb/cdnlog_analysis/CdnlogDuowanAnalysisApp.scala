...
object CdnlogDuowanAnalysisApp extends Logging {
  def main(args: Array[String]) {
    if (args.length != 4) {
      System.err.println("Usage: CdnlogDuowanAnalysisApp <zkQuorum> <group> <topics> < >")
      System.exit(1)
    }
    ...
    val logInfos = ssc.union(kafkaRDD).map(x => { DuowanLogInfo(x._2.trim(), Channel.update()) }).filter { x => x != null }

    val format = new SimpleDateFormat("yyyyMMddHHmm")
    def foreeachRdd = (rdd: RDD[DuowanLogInfo], time: Time) => {
